...
export default function QuizPage() {
    ...
    useEffect(() => {
        ...
        const fetchQuestions = async () => {
            ...
            try {
                ...
                const response = await axios.get(
                    `/api/question/getRandomQuestions/${subject}`,
                    {
                        headers: {
                ...
                localStorage.setItem("quiz_questions", JSON.stringify(fetchedQuestions));
                setLoading(false);
            } catch (error) {
        ...
        if (userId && subject) {
            if (!loadQuizState()) {
                fetchQuestions();
            } else if (localStorage.getItem("quiz_questions")) {
                setLoading(false); // Questions loaded from local
            }
        } else {
    ...
    }, [timeLeft, isQuizCompleted, loading, questions]);
