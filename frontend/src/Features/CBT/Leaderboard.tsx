...
export default function Leaderboard() {
  ...
  const [schoolLeaderboard, setSchoolLeaderboard] = useState<Student[]>([]);
    ...
    try {
      if (activeTab === 'global') {
        ...
        const response = await axios.get(
          `/api/leaderboard/school/${user.school._id}?limit=10`
        );
        setSchoolLeaderboard(response.data.leaderboard);
      } else if (activeTab === 'schools') {
        const response = await axios.get('/api/leaderboard/schools?limit=10');
        setSchoolRankings(response.data.rankings);
      } else if (activeTab === 'myRank' && user) {
        const response = await axios.get(
          `/api/leaderboard/user/${user._id}`,
          {
            headers: {
