...
const TotalResult = () => {
  ...
  useEffect(() => {
    const savedVerifiedUsers = JSON.parse(localStorage.getItem("verifiedUsers") || '[]') || [];
    ...
    const fetchQuizResults = async () => {
      try {
        const response = await axios.get(
          `${import.meta.env.VITE_API_BASE_URL || 'http://localhost:5040'}/api/getAll`
        );
  ...
  const handleVerify = async (userId: string) => {
    try {
      ...
      //   `${import.meta.env.VITE_API_BASE_URL || 'http://localhost:5040'}/api/verify-email`,
      //   { userId }
      // );
