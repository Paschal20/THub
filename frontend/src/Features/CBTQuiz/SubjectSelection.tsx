...
export default function SubjectSelection() {
    ...
    const fetchSubjects = async () => {
      ...
      try {
        const response = await axios.get(
          `${import.meta.env.VITE_API_BASE_URL || 'http://localhost:5040'}/api/kode10x/subjects`
        );
  ...
  const handleContinue = () => {
    ...
    // Store selected subject in sessionStorage for this session only
  ...
  return (
    <div className="space-y-8 py-10">
      <div className="text-center space-y-2">
        <h1 className="text-3xl font-bold text-gray-900">Select Subject</h1>
